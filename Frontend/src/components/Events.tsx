  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        setEvents(data.data); // Changed from data.events to data.data
      } catch (err) {
    ...
    <section id="events" className="py-20 bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-400 font-mono max-w-3xl mx-auto">
            Join us for workshops, competitions, and networking opportunities in
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {events.map((event, index) => {
            const bg = gradientClassOrStyle(event._id || event.title);
            return (
              <div
                key={index}
